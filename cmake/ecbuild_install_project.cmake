...
macro( ecbuild_install_project )
    ...
    # export variables to cache for downstream projects

    else()

      set( ${PNAME}_FOUND             TRUE                          CACHE  BOOL    "${PROJECT_NAME} was found"                      )
      set( ${PROJECT_NAME}_FOUND      TRUE                          CACHE  BOOL    "${PROJECT_NAME} was found"                      )
      set( ${PNAME}_VERSION           ${${PNAME}_VERSION}           CACHE  STRING  "${PROJECT_NAME} version"                        )
      set( ${PROJECT_NAME}_VERSION    ${${PNAME}_VERSION}           CACHE  STRING  "${PROJECT_NAME} version"                        )
      set( ${PNAME}_GIT_SHA1          ${${PNAME}_GIT_SHA1}          CACHE  STRING  "Full Git revision for ${PROJECT_NAME}"          )
      set( ${PNAME}_GIT_SHA1_SHORT    ${${PNAME}_GIT_SHA1_SHORT}    CACHE  STRING  "Abbreviated Git revision for ${PROJECT_NAME}"   )
      set( ${PNAME}_INCLUDE_DIRS      ${${PNAME}_INCLUDE_DIRS}      CACHE  STRING  "${PROJECT_NAME} include directories"            )
      set( ${PNAME}_LIBRARIES         ${${PNAME}_LIBRARIES}         CACHE  STRING  "${PROJECT_NAME} libraries"                      )
      set( ${PNAME}_DEFINITIONS       ${${PNAME}_DEFINITIONS}       CACHE  STRING  "${PROJECT_NAME} compiler defintions"            )
      set( ${PNAME}_PACKAGES          ${${PNAME}_PACKAGES}          CACHE  STRING  "${PROJECT_NAME} packages"                       )
      set( ${PNAME}_TPLS              ${${PNAME}_TPLS}              CACHE  STRING  "${PROJECT_NAME} 3rd party dependencies"         )
      set( ${PNAME}_TPL_INCLUDE_DIRS  ${${PNAME}_TPL_INCLUDE_DIRS}  CACHE  STRING  "${PROJECT_NAME} 3rd party include directories"  )
      set( ${PNAME}_TPL_LIBRARIES     ${${PNAME}_TPL_LIBRARIES}     CACHE  STRING  "${PROJECT_NAME} 3rd party libraries"            )
      set( ${PNAME}_TPL_DEFINITIONS   ${${PNAME}_TPL_DEFINITIONS}   CACHE  STRING  "${PROJECT_NAME} 3rd party compiler definitions" )
      set( ${PNAME}_FEATURES          ${${PNAME}_FEATURES}          CACHE  STRING  "${PROJECT_NAME} features"                       )
      foreach( _f ${${PNAME}_FEATURES} )
        set( ${PNAME}_HAVE_${_f} ${${PNAME}_HAVE_${_f}} CACHE BOOL "${PNAME} has feature ${_f}" )
      endforeach()
