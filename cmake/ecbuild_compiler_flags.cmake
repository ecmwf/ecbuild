...
macro( ecbuild_override_compiler_flags )
    ...
    elseif( DEFINED _PAR_SOURCE_FLAGS OR DEFINED _PAR_COMPILE_FLAGS )
       ...
       if( DEFINED _PAR_COMPILE_FLAGS )
          ...
          set( _skip_compile_flags_include FALSE )
          foreach( _lang C CXX Fortran )
            if( CMAKE_${_lang}_COMPILER_LOADED )
              if( DEFINED ${PNAME}_${_lang}_FLAGS )
                set( _skip_compile_flags_include TRUE )
              endif()

              foreach( _btype IN LISTS _btypelist )
                if( DEFINED ${PNAME}_${_lang}_FLAGS_${_btype} )
                  set( _skip_compile_flags_include TRUE )
                endif()
              endforeach()
            endif()
          ...
          if( _skip_compile_flags_include )
            ecbuild_warn( "Skipping include(${ECBUILD_COMPILE_FLAGS}) because project-specific compiler flags are already defined" )
          else()
            if( _PAR_INHERIT_ECBUILD_FLAGS )
              foreach( _lang C CXX Fortran )
                if( DEFINED ECBUILD_${_lang}_FLAGS )
                  ecbuild_debug( "Initialise ${PNAME}_${_lang}_FLAGS with ECBUILD_${_lang}_FLAGS (${ECBUILD_${_lang}_FLAGS})." )
                  set( ${PNAME}_${_lang}_FLAGS "${ECBUILD_${_lang}_FLAGS}" )
                endif()

                foreach( _btype IN LISTS _btypelist )
                  if( DEFINED ECBUILD_${_lang}_FLAGS_${_btype} )
                    ecbuild_debug( "Initialise ${PNAME}_${_lang}_FLAGS_${_btype} with ECBUILD_${_lang}_FLAGS_${_btype} (${ECBUILD_${_lang}_FLAGS_${_btype}})." )
...
endforeach()
if( DEFINED ${PROJECT_NAME_CAPS}_ECBUILD_COMPILE_FLAGS )
  ecbuild_override_compiler_flags( COMPILE_FLAGS ${${PROJECT_NAME_CAPS}_ECBUILD_COMPILE_FLAGS} WARN INHERIT_ECBUILD_FLAGS )
elseif( DEFINED ${PROJECT_NAME_CAPS}_ECBUILD_SOURCE_FLAGS )
  ecbuild_override_compiler_flags( SOURCE_FLAGS ${${PROJECT_NAME_CAPS}_ECBUILD_SOURCE_FLAGS} WARN )
endif()
