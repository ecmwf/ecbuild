...
# build-on-hpc.
#
# Exists for the platform slug: ecflow has an atos-hpc-nvidia leg, and every consumer
# resolves its deps at its OWN platform, so that leg looks for
# ecbuild-<sha>-atos-hpc-nvidia-Release. The install tree is byte-identical to every
# other ecbuild artifact -- compiler-inputs = [] and there is nothing
# compiler-dependent to install.
#
# NO ctest here, deliberately. atos-hpc-nvidia mirrors the legacy
# ci-hpc-config.yml's `nvidia-24.11` platform, whose ctest_options are
